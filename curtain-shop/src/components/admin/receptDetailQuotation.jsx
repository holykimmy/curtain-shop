...
function ReceptQuotationDetail() {
  ...
  const generatePDF = () => {
    ...
    data.rows.forEach((row, index) => {
      doc.text(`${index + 1}, ${row.list}, ${row.detail}, ${row.width}, ${row.counts}, ${row.unitprice}, ${row.p_width}, ${row.total_m}`, 20, 100 + index * 10);
    });
  
    // Add total price
    doc.text(`ราคารวม : ${numberWithCommas(data.totalPrice || 0)} บาท`, 20, 120 + data.rows.length * 10);
...
<h5 className=" inline-block text-lg md:text-xl xl:text-2xl text-b-font  pl-4 p-2 my-1">
  ใบเสนอราคา
      ...
      <button onClick={generatePDF} className="bg-brown-300 mt-3 mx-2 py-1 px-auto w-[180px] rounded-full shadow-xl hover:bg-brown-200 text-center md:mt-3 md:mb-3 md:inline-blocktext-sm sm:text-xs md:text-xs lg:text-base xl:text-base  text-white">
        พิมพ์ใบเสนอราคา
      </button> <button   onClick={() => handleSwitchtoInvoice(data._id,data.fullname)}className="bg-red-300 mt-3 mx-2 py-1 px-auto w-[180px] rounded-full shadow-xl hover:bg-red-200 text-center md:mt-3 md:mb-3 md:inline-blocktext-sm sm:text-xs md:text-xs lg:text-base xl:text-base  text-white">
        ทำเป็นใบแจ้งหนี้
      </button></div>
    </>
  );
